...
def apply_yolo_object_detection(image_to_process):
    ...
    height, width, za = image_to_process.shape
    ...
    for out in outs:
        for obj in out:
            ...
            if class_score > 0:
                ...
                #print(center_x,center_y,obj_width,obj_height, box)
                boxes.append(box)
                class_indexes.append(class_index)
                #print(class_index)
                class_scores.append(float(class_score))
                #print(float(class_score))

    print(class_scores)
    print(boxes)
    # Selection
    chosen_boxes = cv2.dnn.NMSBoxes(boxes, class_scores, 0.0, 0.4)
    print(list(chosen_boxes))
    for box_index in chosen_boxes:
        ...
        if classes[class_index] in classes_to_look_for:
            objects_count += 1
            #image_to_process = change_object_color(image_to_process,box)
...
def change_object_color(image_to_process,box):

    x, y, w, h = box
    for i in range(y, y + h - 1 if y + h <= image_to_process.shape[0] else image_to_process.shape[0] - 1):
        for j in range(x, x + w - 1 if x + w <= image_to_process.shape[1] else image_to_process.shape[1] - 1):
            pixel = image_to_process[i, j]
            pixel[0] = 255 - pixel[0]
            pixel[1] = 255 - pixel[1]
            pixel[2] = 255 - pixel[2]
            image_to_process[i, j] = pixel
...
if __name__ == '__main__':
    ...
    net = cv2.dnn.readNetFromDarknet("C:/Users/Andrey/Desktop/yolov4.cfg",
                                     "C:/Users/Andrey/Desktop/yolov4.weights")
    layer_names = net.getLayerNames()
    out_layers_indexes = net.getUnconnectedOutLayers()
    out_layers = [layer_names[index - 1] for index in out_layers_indexes]

    # Loading from a file of object classes that YOLO can detect
    with open("C:/Users/Andrey/Desktop/coco.names.txt") as file:
    ...
    video = "C:/Users/Andrey/Desktop/video.mp4"
